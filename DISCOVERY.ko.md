...
아래 매트릭스에서 결론부터 제시합니다. 이후 판정 기준과 제외 범위, 호스트 측 발견, Sandbox 측 발견, 협조 기반 대안과 신뢰하기 어려운 수단을 차례로 다루겠습니다.
...
다음 매트릭스는 통신 방향과 식별 대상, 피어 협조 여부에 따른 결과를 요약합니다. 시험 환경에서 Protected Client는 일반 네트워킹과 같은 결과를 보였습니다.

| 방향 | 식별 대상 | `Networking=Enable`, 피어 협조 없음 | `Networking=Enable`, 피어 협조 있음 | `Networking=Disable` |
| --- | --- | --- | --- | --- |
| 호스트에서 Sandbox | IP 주소 | 휴리스틱 후보만 확인 가능 | 게스트 연결의 원격 주소로 확인 가능 | 사용 불가 |
| 호스트에서 Sandbox | 컴퓨터 이름 | 사용 불가 | 게스트가 이름을 전송하면 확인 가능 | 사용 불가 |
| Sandbox에서 호스트 | IP 주소 | 기본 경로에서 직접 확인 가능 | 호스트 응답으로 확인 및 검증 가능 | 사용 불가 |
| Sandbox에서 호스트 | 컴퓨터 이름 | 사용 불가 | 호스트가 이름을 반환하면 확인 가능 | 사용 불가 |
